...
const XConfig = {
  count: 200,
  colors: [0xff69b4, 0x8ecae6, 0xffffff],
...
/* Helper: create soft green -> white gradient CanvasTexture */
function createSoftGreenToWhiteGradientTexture(size = 256) {
  const canvas = document.createElement("canvas");
  canvas.width = size;
  canvas.height = size;
  const ctx = canvas.getContext("2d")!;
  // diagonal gradient for nice look
  const grad = ctx.createLinearGradient(0, 0, size, size);
  // Soft green -> white
  grad.addColorStop(0, "#b9f6ca"); // soft mint
  grad.addColorStop(0.6, "#e8fff2"); // almost white greenish
  grad.addColorStop(1, "#ffffff"); // white
  ctx.fillStyle = grad;
  ctx.fillRect(0, 0, size, size);

  // subtle radial highlight to give bubble feel
...
/* Helper: create text sprite "S ❤️ G" */
function createSGSprite(text = "S ❤️ G", size = 256) {
  const canvas = document.createElement("canvas");
  canvas.width = size;
  canvas.height = size;
  const ctx = canvas.getContext("2d")!;
  ...
  ctx.font = "bold 80px ui-sans-serif, system-ui, Arial";
  ...
  // draw crisp white text on top (slightly smaller) to make it pop
  ctx.shadowBlur = 0;
  ctx.fillStyle = "#ffffff";
  ctx.font = "bold 72px ui-sans-serif, system-ui, Arial";
  ctx.fillText(text, size / 2, size / 2);
...
class Z extends InstancedMesh {
  ...
  textSprites: Sprite[] = [];
  gradientTexture: CanvasTexture | undefined;
  ...
  constructor(renderer: WebGLRenderer, params: Partial<typeof XConfig> = {}) {
    ...
    // create soft green -> white gradient texture
    const gradientTexture = createSoftGreenToWhiteGradientTexture(512);
    ...
    // create sprites for text — add as children so they'll be transformed by matrix operations
    for (let i = 0; i < config.count; i++) {
      const sprite = createSGSprite();
      // initial scale; will be adjusted in update
      sprite.scale.setScalar(config.minSize * 1.8);
      this.textSprites.push(sprite);
      // add to parent (the InstancedMesh) so it becomes part of scene graph and can be positioned
      this.add(sprite);
    }
  }
  ...
  update(deltaInfo: { delta: number }) {
    ...
    for (let idx = 0; idx < this.count; idx++) {
      ...
      // size from physics
      const sizeScalar = this.physics.sizeData[idx] || 1;
      // apply scale to instance matrix
      U.scale.setScalar(sizeScalar);
      U.updateMatrix();
      this.setMatrixAt(idx, U.matrix);

      // update corresponding sprite position & scale
      const sprite = this.textSprites[idx];
      if (sprite) {
        // copy world position from instance (U.position)
        sprite.position.copy(U.position);
        // move slightly in front of sphere so it's visible and not z-fighting
        sprite.position.z += 0.12 * Math.max(0.5, sizeScalar);
        // scale relative to sphere size so text remains readable
        const spriteScale = Math.max(0.8, sizeScalar * 1.6);
...
function createBallpit(
  ...
  function initialize(cfg: any) {
    if (spheres) {
      // dispose previous spheres & textures cleanly
      try {
        spheres.geometry.dispose();
        // dispose material map if canvas texture
        if ((spheres.material as any).map) {
          (spheres.material as any).map.dispose();
        }
    ...
    // add spheres to scene (they are an InstancedMesh)
